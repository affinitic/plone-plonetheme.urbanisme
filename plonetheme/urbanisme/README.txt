...
First let's check that the view exists
    ...
    >>> from plonetheme.urbanisme.browser.urbanismeview import themeView
    >>> self.view = ThemeView(self.portal, self.app.REQUEST)
...
visible.
    ...
    >>> from plonetheme.urbanisme.tests.base import zcml_string
    >>> zcml.load_string(zcml_string)
    >>> portal_setup = self.portal.portal_setup
    >>> portal_setup.runAllImportStepsFromProfile('profile-plonetheme.urbanisme:testing')
    {...}
    >>> browser.reload()
    >>> 'id="portal-column-one"' in browser.contents
    False
