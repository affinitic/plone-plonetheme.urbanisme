...
class Layer(BasePTCLayer):
    ...
    def afterSetUp(self):
        ...
        from plonetheme import urbanisme
        zcml.load_config('configure.zcml', package=urbanisme)
        fiveconfigure.debug_mode = False
        # after which the required packages can be initialized
        installPackage('plonetheme.urbanisme', quiet=True)
        # finally load the testing profile
        self.addProfile('plonetheme.urbanisme:default')
...
<configure xmlns="http://namespaces.zope.org/zope"
           ...
           package="plonetheme.urbanisme"
           i18n_domain="test">

    <gs:registerProfile
        name="testing"
        title="plonetheme.urbanisme testing"
...
class ThemeurbanismeTestCase(Sandboxed, ptc.PloneTestCase):
